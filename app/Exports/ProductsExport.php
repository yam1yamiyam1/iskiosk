...
class ProductsExport implements FromCollection, WithHeadings, ShouldAutoSize, WithStyles
{
    ...
    public function collection()
    {
        ...
        return $products->map(function ($product) {
            ...
            foreach ($this->sortedShops as $shop) {
                $slug = $shop->slug . '-quantity';
                $quantity = $product->shops->firstWhere('id', $shop->id)?->pivot?->quantity ?? 0;
                $data[$slug] = $quantity;
            }
    ...
    public function headings(): array
    {
        ...
        $shopHeaders = $this->sortedShops->map(fn($shop) => $shop->slug . '-quantity')->toArray();
