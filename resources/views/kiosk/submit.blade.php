...
<div class="form-container">
  ...
  <form id="submitForm" action="{{ route('kiosk.store') }}" method="POST" novalidate>
    @csrf
...
async function handleScan(scannedID) {
  ...
  try {
    const res=await fetch('{{ route('kiosk.verifyID') }}',{method:'POST',headers:{'Content-Type':'application/json','X-CSRF-TOKEN':'{{ csrf_token() }}'},body:JSON.stringify({id_number:scannedID})});
